...
void Buttons_isr(void* context, alt_u32 id){
	...
	printf("button Binary/Decimal value: %i\n", buttonValue);
...
void init_buttons_pio(){
  ...
  // register the ISR
  alt_irq_register(BUTTONS_IRQ, NULL, Buttons_isr);
...
//Check Switch Press for Mode Switching in a safe state and sets button Mask
void initMode(){

	uint8_t S0=(IORD_ALTERA_AVALON_PIO_DATA(SWITCHES_BASE)&Switch0)&Switch0;
	uint8_t S1=(IORD_ALTERA_AVALON_PIO_DATA(SWITCHES_BASE)&Switch1)&Switch1;
